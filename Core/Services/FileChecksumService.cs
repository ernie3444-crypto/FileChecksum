    ...
    public partial class FileChecksumService : ServiceBase
    {
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                // 5. Apply drive override if specified
                if (!string.IsNullOrEmpty(config.DriveOverride))
                {
                    try
                    {
                        config = _serviceContext.ConfigurationService.ApplyDriveOverride(config, config.DriveOverride);
                        WriteEventLog($"Applied drive override: {config.DriveOverride}. File paths updated.");
                        
                        // Show sample of updated paths for verification
                        if (config.Files.Count > 0)
                        {
                            WriteEventLog($"Example file path: {config.Files[0].Path}");
                        }
                    }
                    catch (Exception ex)
                    {
                        WriteEventLog($"Error applying drive override: {ex.Message}", EventLogEntryType.Error);
                        throw;
                    }
                }
                ...
                // 7. Set up periodic checksum verification
